...
脚本会：
1. 下载 neetcode.io 的 main.js bundle
2. 从中提取所有题目数据（title、slug、difficulty、topic、75/150/250 标志）
3. 覆盖写入 `seed_data/neetcode_problems.json`

### 第二步：加载到数据库

在网页 UI 右上角点击 **↻ Sync** 按钮，或重启后端。

同步逻辑：
- 新题 → 自动添加
- 已有题 → 更新元数据（difficulty、topic、标志）
- 官网删除的题 → 从数据库移除（连带清理 progress/review_log/notes）
- **不影响已有的刷题进度和笔记**
